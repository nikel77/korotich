...
def str_then_dict(input):
    ...
    if input[1] != '{':
        ...
        for i in range(0, len(input)-1):
            if ';' not in input[i] and ';' not in input[i+1] and input[i] != '{' and input[i] != '}' and input[i+1] != '{' and input[i+1] != '}':
                 filtered_input.append(input[i] + input[i+1])
            elif input[i+1] == '{':
                continue
            else:
                filtered_input.append(input[i])
        dict = list_then_dict(filtered_input)
    else:
        dict = list_then_dict(input)
    return dict
